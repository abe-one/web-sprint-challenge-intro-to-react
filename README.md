...
1. What is React JS and what problems does it solve? Support your answer with concepts introduced in class and from your personal research on the web.
   [x] React is library used to build web apps without having to manage or recreate every minor detail of DOM manipulation. Instead of reloading the entire page when something changes, React only reload the the nodes that have been changed by some event or process. React, and others libraries like it, ease pressure off browsers and developers.

1. Describe component state.
   [x] State is the status of something. In react, state specific data held for use by components. When the state is changed, (data update) react will rerender the component using that data.

1. Describe props.
   [x] Props are read only data passed along to another component. The props are read by the lower components so that the data remains synced with the ultimate value of the props determined by the parent component. Examples of data passed down as props include state data, set state functions, functions.

1. What are side effects, and how do you sync effects in a React component to changes of certain state or props?
   [x] Side effects are anything that changes something outside the scope of the function causing the changes. Fetching data, logging, manually manipulating the DOM. Because side effects don't affect the return of the function invoking them, the useEffect hook allows side effects to be run every time a chosen state(s) is set , or simply on first render. That way the side effect is selectively invoked, rather than invoked on every occasion that the parent function is invoked.
